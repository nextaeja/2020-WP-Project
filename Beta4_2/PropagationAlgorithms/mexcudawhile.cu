...
void mex_split_operator_step_3rd_vsplit_time_dependent(double t_query, myComplex* dev_expv, double* dev_z_offset, double* dev_gauss_pos, double* dev_x0, double* dev_y0, myComplex* dev_expk, myComplex* dev_psi, int nx, int ny, int nz, int decay_type, double A, double eV, double h_bar, double dt, double dx, double dy, double dz, double alpha, int iteration, int num_adsorbates) {
	...
	split_operator_3rd_vsplit_time(dev_psi, dev_expv, dev_expk, dev_x0, dev_y0, dev_z_offset, t_query, A, eV, expv_scale, grid_size, forward_plan, inverse_plan, num_adsorbates, nx, ny, nz, decay_type, dx, dy, dz, dt,alpha);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	int iteration = mxGetScalar(prhs[21]);
	int num_adsorbates = mxGetScalar(prhs[22]);
